// =================================================================================
//  項目: Flux AI Pro - Ultimate Edition (v9.9.1)
//  功能: 全功能 UI + IndexedDB 永久存檔 + 智能優化開關 + 45種風格
// =================================================================================

// =================================================================================
//  第一段: 配置與工具類 (Config & Utils)
// =================================================================================

const CONFIG = {
  PROJECT_VERSION: "9.9.1-pro",
...
async function translateToEnglish(text) {
  try {
    ...
    if (data && data[0] && Array.isArray(data[0])) { 
        data[0].forEach(segment => { if (segment && segment[0]) translatedText += segment[0]; }); 
    }
...
class ParameterOptimizer {
  static optimizeSteps(model, width, height, style, qualityMode = 'standard') {
    ...
    if (totalPixels >= rules.SIZE_MULTIPLIER.xlarge.threshold) sizeMultiplier = rules.SIZE_MULTIPLIER.xlarge.multiplier;
    else if (totalPixels >= rules.SIZE_MULTIPLIER.large.threshold) sizeMultiplier = rules.SIZE_MULTIPLIER.large.multiplier;
    ...
    let optimizedSteps = Math.round(baseSteps * sizeMultiplier * qualityMultiplier);
    return Math.max(modelRule.min, Math.min(optimizedSteps, modelRule.max));
  }
  
  static optimize({ prompt, negative_prompt, model, width, height, quality_mode, auto_optimize, auto_hd, style, seed, reference_images }) {
    let finalPrompt = prompt;
    let finalNegative = negative_prompt || "";
    let finalWidth = width;
    let finalHeight = height;
    let finalSteps = null;
    let finalGuidance = null;
    
    // 1. 自動高清 (Auto HD)
    if (auto_hd) {
        const modeConfig = CONFIG.HD_OPTIMIZATION.QUALITY_MODES[quality_mode] || CONFIG.HD_OPTIMIZATION.QUALITY_MODES.standard;
        const hdLevel = modeConfig.hd_level;
        
        // 注入 Prompt
        if (CONFIG.HD_OPTIMIZATION.HD_PROMPTS[hdLevel]) {
            finalPrompt += `, ${CONFIG.HD_OPTIMIZATION.HD_PROMPTS[hdLevel]}`;
        }
        // 注入 Negative
        if (CONFIG.HD_OPTIMIZATION.HD_NEGATIVE) {
            finalNegative += finalNegative ? `, ${CONFIG.HD_OPTIMIZATION.HD_NEGATIVE}` : CONFIG.HD_OPTIMIZATION.HD_NEGATIVE;
        }
        
        // 尺寸優化
        if (modeConfig.force_upscale || Math.min(width, height) < modeConfig.min_resolution) {
            // 這裡簡單處理，實際可做更複雜的等比縮放
            // finalWidth = ...
        }
    }
    
    // 2. 參數優化 (Auto Optimize)
    if (auto_optimize) {
        finalSteps = this.optimizeSteps(model, finalWidth, finalHeight, style, quality_mode);
        // Guidance 簡單邏輯
        finalGuidance = (model.includes('turbo') || style === 'photorealistic') ? 3.5 : 7.5;
        if (quality_mode === 'ultra') finalGuidance += 1.0;
    }
    ...
    if (style && CONFIG.STYLE_PRESETS[style]) {
        ...
        if (s.negative) finalNegative += finalNegative ? `, ${s.negative}` : s.negative;
    }
...
class PollinationsProvider {
  ...
  async generate(prompt, options, logger) {
    ...
    params.append('enhance', options.auto_hd || false); // 使用 auto_hd 控制 enhance
...
// =================================================================================
//  第三段: 前端 Web UI (含 智能優化開關 + IndexedDB 永久存檔)
// =================================================================================
...
<div class="main" id="page-generate">
  ...
  <div class="panel settings">
    ...
    <div class="form-group"><label>數量 (Batch)</label><input type="number" id="count" value="1" min="1" max="4"></div>
    
    <!-- 方案 A：整合進階面板 -->
    <div class="form-group">
      <label>智能優化 (Smart AI)</label>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        <label style="flex:1;display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.05);padding:8px;border-radius:6px;cursor:pointer;border:1px solid var(--border);font-size:12px">
            <input type="checkbox" id="autoOpt" checked> ⚡ 參數優化
        </label>
        <label style="flex:1;display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.05);padding:8px;border-radius:6px;cursor:pointer;border:1px solid var(--border);font-size:12px">
            <input type="checkbox" id="autoHD" checked> 💎 Auto HD
        </label>
      </div>
      <div style="font-size:10px;color:#64748b;margin-top:4px;line-height:1.4">
        ⚡ 自動調整 Steps/Guidance<br>
        💎 自動注入 8k/Masterpiece 提示詞
      </div>
    </div>
  </div>
</div>
    ...
    try {
        const res = await fetch('/_internal/generate', {
            ...
            body: JSON.stringify({
                ...
                // ✨ 傳遞優化參數
                auto_optimize: document.getElementById('autoOpt').checked,
