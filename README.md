...
- **🌍 多語言擴展**：新增日文、韓文、阿拉伯語支援，總共支援 5 種語言（繁體中文、英文、日文、韓文、阿拉伯語）。
- **🌐 自動偵測系統語言**：新增「自動偵測」選項，根據瀏覽器語言自動切換介面語言，提供更佳的用戶體驗。
- **🔄 RTL 語言支援**：完整支援阿拉伯語等 RTL（從右到左）語言，自動調整文字方向與佈局。
...
- **日本語 (ja)**：完整日文介面
- **한국어 (ko)**：完整韓文介面
- **العربية (ar)**：完整阿拉伯語介面（RTL 支援）

#### 語言切換方式
- 點擊介面右上角的語言按鈕即可切換語言
- 選擇 **「🌐 自動偵測」** 選項，系統會根據瀏覽器語言自動切換介面語言
- 切換後所有 UI 元素會即時更新為選擇的語言
- 語言設定會保存在 localStorage 中，下次訪問時自動套用

#### RTL 語言支援
- 阿拉伯語等 RTL 語言會自動調整文字方向與佈局
- 所有 UI 元素都經過 RTL 優化，確保正確顯示

#### 已翻譯的 UI 元素

所有 UI 元素都已完整翻譯為 5 種語言（繁體中文、英文、日文、韓文、阿拉伯語）：

**主頁面：**
- 導航選項：生成圖像、歷史記錄、Nano版
- 設定標籤：生成參數、API 供應商、模型選擇、尺寸、模型選擇、尺寸預設、藝術風格、質量模式
- 進階設定：自動優化、進階參數、批量生成
